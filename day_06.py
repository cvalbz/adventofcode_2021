...
def challenge_a(fish, days):
    for _ in range(days):
        new_fish = len([i for i in fish if i == 0])
...
def solve_challenge_a(data, days):
    fish = parse_data(data)
    return challenge_a(fish, days)

challenge_test_data = [3,4,3,1,2]

assert challenge_a(challenge_test_data, 18) == 26
assert challenge_a(challenge_test_data, 80) == 5934
...
@functools.lru_cache()
def count_fish(f, days):
    if days == 0:
        return 1

    if f > 0:
        return count_fish(f - 1, days - 1)

    return count_fish(6, days - 1) + count_fish(8, days - 1)

def challenge_b(fish, days):
    fish_count = sum([count_fish(f, days) for f in fish])
    return fish_count

def solve_challenge_b(data, days):
    fish = parse_data(data)
    return challenge_b(fish, days)

assert challenge_b(challenge_test_data, 256) == 26984457539 

if __name__ == "__main__":
    DAY = 6
    puzzle_a(DAY, lambda data: solve_challenge_a(data, 80)) # 393019 
    puzzle_b(DAY, lambda data: solve_challenge_b(data, 256)) # 1757714216975 
