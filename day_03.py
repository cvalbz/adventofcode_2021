...
from utils import quantify, transpose

def challenge_a(diagnostics):
    diag_trans = transpose(diagnostics)
    dimension = len(diag_trans[0])

    gamma = [1 if sum(i) > dimension // 2 else 0 for i in diag_trans]
...
def parse_data(data):
    ...
    diagnostics = [map(int, i) for i in lines]
    return diagnostics
...
00010
01010"""
assert solve_challenge_a(challenge_test_data) == 198
...
(2) than 0 bits (3), so keep only the 2 numbers with a 1 in the second position: 01111 and 01010.
...
def filter_rec(iterable, majority_func, position):
    assert len(iterable) > 0

    if len(iterable) == 1 or position == len(list(iterable[0])):
        return iterable[0]
    
    bits = [i[position] for i in iterable]
    m = majority_func(bits)

    it = [x for (ix, x) in enumerate(iterable) if bits[ix] == m]
    return filter_rec(it, majority_func, position + 1)

def challenge_b(diagnostics):
    diagnostics = [list(i) for i in diagnostics]
    oxygen = filter_rec(diagnostics, lambda x: 1 if sum(x) >= (len(x)+1) // 2 else 0, 0)
    oxygen = int("".join(map(str, oxygen)), 2)

    co2 = filter_rec(diagnostics, lambda x: 1 if sum(x) < (len(x)+1) // 2 else 0, 0)
...
assert solve_challenge_b(challenge_test_data) == 230

if __name__ == "__main__":
    DAY = 3
    puzzle_a(DAY, solve_challenge_a) # 2954600
    puzzle_b(DAY, solve_challenge_b) # 1662846
