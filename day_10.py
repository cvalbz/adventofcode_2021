...
chunk (if any) can immediately start. Every chunk must open and close with one of
...
MATCHING_REV = dict([(j, i) for i, j in MATCHING.items()])
    ...
    for line in data:
        ...
        for p in line:
            if p in "([{<":
                ...
                top = stack.pop() # no unbalanced input data
                if top != MATCHING[p]:
                    errors_sum += ERROR_POINTS[p]
                    break
            else:
...
assert solve_challenge_a(challenge_test_data) == 26397
...
def challenge_b(data):
    ...
    for line in data:
        ...
        if not error and len(stack) > 0: # incomplete line
            score = 0

            for p in reversed(stack):
                score = score * 5 + INCOMPLETE_POINTS[MATCHING_REV[p]]
...
assert solve_challenge_b(challenge_test_data) == 288957

if __name__ == "__main__":
    DAY = 10
    puzzle_a(DAY, solve_challenge_a) # 216297
    puzzle_b(DAY, solve_challenge_b) # 2165057169
